...
\begin{example}
  ...
  Using the applet, set $\vect{u}=\startmat{r} -2 \\ 1 \stopmat$ and $\vec{w}=\startmat{r} 2 \\ 2 \stopmat$ as new spanning vectors for $\RR^2$.
  
  What linear combination of $\vec{u}$ and $\vec{w}$ makes up $\vec{v}=[2,3]$?
  ...
  \begin{solution}
    ...
    $\vec{v}=\answer[tolerance=.1]{.33}\vec{u}+\answer[tolerance=.1]{1.33}\vec{w}$.
