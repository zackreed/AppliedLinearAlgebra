...
\begin{example}
  ...
  Using the applet, Check the "Set New Span Vectors" box and set $\vect{b_1}=\startmat{r} -2 \\ 1 \stopmat$ and $\vec{b}_2=\startmat{r} 2 \\ 2 \stopmat$ as new spanning vectors for $\RR^2$. Then check "Find v in Current Span" and set the vector $\vec{v}$ to $[2,3]$.
  
  What linear combination of $\vec{b}_1$ and $\vec{b}_2$ makes up $\vec{v}$? (Select the "New Vectors" button to see the new span)
  ...
  \begin{solution}
    ...
    $\vec{v}=\answer[tolerance=.1]{.33}\vec{b}_1+\answer[tolerance=.1]{1.33}\vec{b}_2$.
...
\begin{example}\name{Spanning Multiple Vectors}
  ...
  Now, use the applet to find the linear combinations of the vectors $\vec{b}_1$ and $\vec{b}_2$ that make up the vectors $\vec{v}$:

  \begin{enumerate}
  
    \item $\vec{b}_1=[1,2]$, $\vec{b}_2=[2,1]$, $v=[4,5]$
    
    $$\vec{v}=\answer{2}\vec{b}_1+\answer{1}\vec{b}_2$$

    \item $\vec{b}_1=[1,2]$, $\vec{b}_2=[-2,1]$,
    
    $$\answer{-1}\vec{b}_1+\answer{3}\vec{b}_2=[-7,1].$$
...
According to MATLAB, $\vec{w}$ is in the span of $\vec{u}$ and $\vec{v}$ via the linear combination

$$\vec{w}=\answer{5}\vec{u}+\answer{-1}\vec{v}.$$
...
There's no solution, so \verb|z| \wordChoice{\choice{is}\choice[correct]{is not}} in the span of \verb|u| and \verb|v|.
