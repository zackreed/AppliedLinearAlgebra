...
\end{example}
  ...
  This yields $\answer{-1/2} u_1 + \answer{0} u_3 = u_2$, and $\answer{2/3} u_1 + \answer{5/3} u_3 = u_4$. This means that $u_1$ and $u_3$ are \wordChoice{\choice{linearly dependent}\choice[correct]{linearly independent}}, so we need them in the basis. Moreover, we were able to make $u_2$ and $u_4$ from $u_1$ and $u_3$, so we don't need them in the basis.
...
\begin{exercise}
  ...
  \begin{enumerate}
    ...
    \item $\vect{u}_1=\startmat{r} 1 \\ 1 \\ 1 \stopmat$, $\vect{u}_2=\startmat{r} 1 \\ 1 \\ 0 \stopmat$, $\vect{u}_3=\startmat{r} 1 \\ 0 \\ 0 \stopmat$.
      ...
      Note that since $\vec{u}_1$ is the only vector with a $1$ in the third component, you \wordChoice{\choice{can}\choice[correct]{cannot}} make $\vec{u}_1$ from a linear combination of $\vec{u}_2$ and $\vec{u}_3$. Using similar reasoning, you'll note that it is \wordChoice{\choice{possible}\choice[correct]{impossible}} to construct any of $\vec{u}_1$, $\vec{u}_2$ or $\vec{u}_3$ from linear combinations of the other two. Thus, the vectors are \wordChoice{\choice{linearly dependent}\choice[correct]{linearly independent}}. 
      
      Thus, $\sspan\set{\vect{u}_1,\vect{u}_2,\vec{u}_3}=\R^3$, since they are 3 $3$-dimensional linearly independent vectors. This technically makes a \wordChoice{
        \choice{point}
        \choice{line}
        \choice{plane}
    ...
    \item $\vect{u}_1=\startmat{r} 1 \\ -1 \\ 1 \stopmat$, $\vect{u}_2=\startmat{r} 2 \\ 0 \\ 1 \stopmat$, $\vect{u}_3=\startmat{r} 1 \\ 1 \\ 0 \stopmat$.
      ...
      If you use MATLAB or reason about the ways to combine the vectors, you note that $\answer{1}\vec{u}_1+\answer{-1}\vec{u}_2=\vec{u}_3$, so $\vec{u}_1$ and $\vec{u}_2$ are \wordChoice{\choice[correct]{linearly dependent}\choice{linearly independent}}, and $\vec{u}_3$ is \wordChoice{\choice{also linearly independent}\choice[correct]{redundant}}.
