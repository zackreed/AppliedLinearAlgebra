...
\title{Inner Product Spaces and Orthogonal Matrices}
\begin{document}
\begin{abstract}

    This covers the properties of Orthogonal matrices
...
\begin{solution}
  ...
  \begin{equation*}
    \startmat{c} a \\ b \\ c \\ d \stopmat
    = \startmat{c} 1 \\ 0 \\ 0 \\ 0 \stopmat,\quad
    \startmat{c} a \\ b \\ c \\ d \stopmat
    = \startmat{c} 0 \\ 5 \\ 0 \\ -3 \stopmat,\quad
    \startmat{c} a \\ b \\ c \\ d \stopmat
...
\begin{example}{Orthogonal, orthonormal, and non-orthogonal bases}{orthogonal-basis}
  ...
  \begin{itemize}
  \item $\set{\startmat{r} 1 \\ 0 \stopmat,
      \startmat{r} 0 \\ 1 \stopmat}$ is an orthonormal
    basis of $\R^2$.
  \item
    $\set{\frac{1}{\sqrt{2}}\startmat{r} 1 \\ 1 \stopmat,
      \frac{1}{\sqrt{2}}\startmat{r} 1 \\ -1 \stopmat}$
    is an orthonormal basis of $\R^2$.
  \item $\set{\startmat{r} 2 \\ 0 \stopmat,
      \startmat{r} 0 \\ 3 \stopmat}$ is an orthogonal
    basis of $\R^2$, but not orthonormal.
  \item $\set{\startmat{r} 1 \\ 1 \stopmat,
      \startmat{r} 1 \\ -1 \stopmat}$ is an orthogonal
    basis of $\R^2$, but not orthonormal.
  \item $\set{\startmat{r} 1 \\ 0 \stopmat,
      \startmat{r} 1 \\ 1 \stopmat}$ is a basis of
...
\begin{example}{Finding partial coordinates}{partial-coordinates}
  ...
  \begin{equation*}
    \vect{u}_1 = \startmat{c} 1 \\ 2 \\ -1 \\ 0 \stopmat
    \quad\mbox{and}\quad
    \vect{u}_2 = \startmat{c} 0 \\ 1 \\ 2 \\ 3 \stopmat,
  \end{equation*}
  but it is not known what $\vect{u}_3$ and $\vect{u}_4$ are. Find the
  first two coordinates of the vector
  \begin{equation*}
    \vect{v} = \startmat{c} 1 \\ 0 \\ 2 \\ 0 \stopmat
  \end{equation*}
...
\begin{proof}
  ...
  \begin{equation*}
    \coord{\vect{v}}_B
    = \startmat{c} a_1 \\ \vdots \\ a_n \stopmat
    \quad\mbox{and}\quad
    \coord{\vect{w}}_B
    = \startmat{c} b_1 \\ \vdots \\ b_n \stopmat,
  \end{equation*}
