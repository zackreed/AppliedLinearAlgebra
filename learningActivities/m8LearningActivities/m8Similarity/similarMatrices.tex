...
\end{bmatrix}$ similar?
\begin{explanation}
A quick check shows us $\det A$ and $\det B$ \wordChoice{\choice[correct]{are}\choice{are not}} equal, and both matrices are seen to \wordChoice{\choice[correct]{be invertible}\choice{not be invertible}}, so they have the \wordChoice{\choice[correct]{same}\choice{different}} rank.  However, $\mbox{tr} A = \answer{1}$ and $\mbox{tr} B = \answer{2}$, so the matrices \wordChoice{\choice{are}\choice[correct]{are not}} similar.
\end{explanation}
\end{example}
